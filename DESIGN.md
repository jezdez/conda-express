...
On the first conda command, `cx` installs conda into the target prefix, writes
prefix metadata, configures the selected solver and plugins, freezes the base
prefix, and delegates the original arguments. Later invocations delegate
directly to the installed `conda` executable.
...
- `cx spawn` as the conda-spawn based activation command
...
When the managed prefix is absent, the runtime automatically:

1. Determines the target prefix from its stamped policy and `CX_PREFIX`.
2. Validates the stamped lock and the `CX_BUNDLE` and `CX_OFFLINE` controls.
...
It then runs the installed conda executable with the original arguments.
