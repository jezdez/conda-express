...
cx takes a different approach: a single ~17 MB Rust binary that bootstraps a
...
3. Pre-built platform wheels (~17 MB each) are uploaded to PyPI for every target
